...
<table width="995" cellspacing="0" cellpadding="2" border="0">
    <tr>
        <td align="center" valign="top" bgcolor="#E2E2E2">
            <br />
            <?php
			if (!empty($_POST['jenis'])) { $jenis = $_POST['jenis']; }
			else { $jenis = "ALL"; }
            ...
            <table width="990" border="0" cellspacing="0" cellpadding="3">
              ...
              <tr bgcolor="#E2E2E2">
                ...
                <td width="46" align="right"><select name="jenis" id="jenis">
                  <option value="">- SEMUA -</option>
                  <option value="1" <?php if ($jenis == 1) { echo 'selected="selected"'; } ?>>KURSUS/LATIHAN</option>
                  <option value="2" <?php if ($jenis == 2) { echo 'selected="selected"'; } ?>>SESI PEMBELAJARAN</option>
                  <option value="3" <?php if ($jenis == 3) { echo 'selected="selected"'; } ?>>PEMBELAJARAN KENDIRI</option>
                </select></td>
          ...
          <div align="right" class="text1">
			 ...
			 if (isset($_POST['Submit'])) {
				 ?>
	             [ <a href="export.php?menu=jenis<?php if ($jenis != "ALL") { echo "&jenis=".$jenis; } ?><?php if ($status != "ALL") { echo "&status=".$status; } ?><?php if ($bulan != "ALL") { echo "&bulan=".$bulan; } ?>&tahun=<?php echo $tahun; ?>">Export</a> ] 
             <?php
			 }
			 else {
				 ?>
	             [ <a href="export.php?menu=jenis">Export</a> ] 
             <?php
			 }
			 ?>
          </div>
          ...
          <table width="990" border="1" cellspacing="0" cellpadding="3" style="border-collapse: collapse">
			...
			if ($jenis == "ALL") {
				?>
                <tr class="text2">
                  <td colspan="8" align="center" bgcolor="#DE5302">LAPORAN MENGIKUT JENIS KURSUS</td>
                </tr>
                <tr class="text1">
                  <td width="40" align="center" valign="middle" bgcolor="#CCCCCC">Bil</td>
                  <td width="161" align="left" valign="middle" bgcolor="#CCCCCC">Nama 
					&amp; No. KP</td>
                  <td width="123" align="center" valign="middle" bgcolor="#CCCCCC">Unit</td>
            ...
            <?php
				...
				?>
                <tr class="text2">
                  <td colspan="5" align="center" bgcolor="#DE5302">LAPORAN MENGIKUT JENIS KURSUS</td>
                </tr>
                <tr class="text1">
                  <td width="40" align="center" valign="middle" bgcolor="#CCCCCC">Bil</td>
                  <td width="302" align="left" valign="middle" bgcolor="#CCCCCC">Nama 
					&amp; No. KP</td>
                  <td width="219" align="center" valign="middle" bgcolor="#CCCCCC">Unit</td>
                  <td width="205" align="center" valign="middle" bgcolor="#CCCCCC">Kumpulan Perkhidmatan</td>
                  <td width="182" align="center" valign="middle" bgcolor="#CCCCCC">
                  	<?php
                    if ($jenis == 1) { echo "Kursus/Latihan"; }
                    elseif ($jenis == 2) { echo "Sesi Pembelajaran"; }
					else { echo "Pembelajaran Kendiri"; }
					?>
                  </td>
                </tr>
            <?php
			...
			while ($t = mysql_fetch_array($d)) {
				...
				if ($jenis == "ALL") {
					$totalx = $kursus + $sesi + $kendiri;
					if ($status != "ALL" && $totalx <= 0) { $zaki = FALSE; }
					else { $zaki = TRUE; }
					if ($zaki) {
						$n = $n + 1;
						?>
					...
					if ($jenis == 1) { $totalx = $kursus; }
					elseif ($jenis == 2) { $totalx = $sesi; }
					elseif ($jenis == 3) { $totalx = $kendiri; }
					if ($status != "ALL" && $totalx <= 0) { $zaki = FALSE; }
					else { $zaki = TRUE; }
					if ($zaki) {
						$n = $n + 1;
						...
						<tr>
						  <td align="center" valign="middle"><?php echo $n; ?></td>
						  <td align="left" valign="middle"><?php echo output1($t['unama']) . "<br><a href=\"index.php?modul=laporan&menu=kakitangan&id=".$t['uids']."\">" . output1($t['unokp']) . "</a>"; ?></td>
						  <td align="center" valign="middle"><?php echo output1($t['bahagian']); ?></td>
						  <td align="center" valign="middle"><?php echo output1($t['kumpulan']); ?></td>
						  <td align="center" bgcolor="#CCCCFF">
							<?php 
			...
			if ($n) {
				if ($jenis == "ALL") {
					?>
					<tr class="text1">
					  <td colspan="4" align="center" bgcolor="#00FFFF">Jumlah Keseluruhan Hari</td>
					  <td colspan="4" align="center" bgcolor="#00FFFF">
						<?php 
						$hari = floor($ttotal / 6);
						$jam = $ttotal - ($hari * 6);
						if ($hari >= 1) { $sesid = $hari . " HARI " . $jam . " JAM"; }
